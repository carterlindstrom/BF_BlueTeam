...
public class TestMovieAndMovieDataReader {
  ...
  public void runTests() {
    ...
    System.out.println("//////////My test are below////////////");
    ...
    if(this.testMovieGetDescription()){
            System.out.println("Test Movie getDescription: PASSED");
    }else{
            System.out.println("Test Movie getDescription: FAILED");
    }
  }
  ...
  // TODO: Data Wrangler, add at least 2 more tests
          /**
   * check if each movie read has the right year 
  ...
  public boolean testMovieYear(){
      ...
      if (!movieList.get(0).getYear().equals(year1))
                // test failed
                return false;

      if (!movieList.get(1).getYear().equals(year2)) 
          return false;
      if (!movieList.get(2).getYear().equals(year3)) 
          return false;
        // all tests passed
      return true;
  }
  ...
  public boolean testMovieGetDescription(){
      List<MovieInterface> movieList;
      readerToTest = new MovieDataReader();
      try {
        movieList = readerToTest.readDataSet(new StringReader(
      ...
      String d1 = "A series of stories woven together by one of our most primal fears, the fear of the unknown.";
      String d2 = "The director of the largest media company wants to expose how left-wing powers use film to control populations.";
      String d3 = "Set to a new wave '80s soundtrack, a pair of young lovers from different backgrounds defy "
              + "their parents and friends to stay together. A musical adaptation of the 1983 film.";
      
          
    
          
      if (!movieList.get(0).getDescription().equals(d1)) {
          // test failed
        System.out.println("implemetation des: "+ movieList.get(0).getDescription());
          return false;
      }
  
      if (!movieList.get(1).getDescription().equals(d2)) {
        System.out.println("implemetation des: "+ movieList.get(1).getDescription());
          return false;
      }
      if (!movieList.get(2).getDescription().equals(d3)) {
        System.out.println("implemetation des: "+ movieList.get(1).getDescription());
          return false;
          // all tests passed
      }
